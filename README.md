...
The **Museum With No Frontiers — Amulets and Talismans** gallery: the gallery
...
| Package | Role |
| --- | --- |
| `@metanull/amulets-data` | the dataset (JSON + `manifest.json`, **private**) |
| `@metanull/viewer-core` | application engine (routing, data access, the text runtime and the language service) |
| `@metanull/viewer-i18n` | the texts shared with the other MWNF websites (this one receives the `gallery` bundle) |
...
<https://metanull.github.io/amulets/> through the reusable workflows in
